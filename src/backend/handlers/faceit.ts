...
async function ensureDeveloperDebugProfile() {
  ...
  if (!profile?.player || !profile.playerId) {
...
function allocateKills(players: MatchPlayer[], userId: number, outcome: 'W' | 'D' | 'L', style: 'DEFAULT' | 'MVP' | 'BOTTOM', isUserTeam: boolean) {
  const total = outcome === 'D'
    ? rand(90, 115)
    : isUserTeam
      ? outcome === 'W' ? rand(95, 120) : rand(70, 95)
      : outcome === 'W' ? rand(70, 95) : rand(95, 120);
...
function buildVictimPicker(players: MatchPlayer[], userId: number, userWeight: number) {
  ...
  return () => {
    const expanded = Array.from(weights.entries()).flatMap(([id, weight]) => Array(Math.max(1, weight)).fill(id));
...
function createSyntheticEvents(
  ...
  const userDeathWeight = style === 'BOTTOM' && outcome === 'L' ? 4 : style === 'MVP' && outcome === 'W' ? 1 : 2;
  ...
  yourTeam.forEach((player) => {
    ...
    for (let idx = 0; idx < kills; idx += 1) {
      ...
      const victim = oppTeam.find((entry) => entry.id === victimId) || oppTeam[rand(0, oppTeam.length - 1)];
  ...
  oppTeam.forEach((player) => {
    ...
    for (let idx = 0; idx < kills; idx += 1) {
      ...
      const victim = yourTeam.find((entry) => entry.id === victimId) || yourTeam[rand(0, yourTeam.length - 1)];
...
export default function registerFaceitHandlers() {
  ...
  ipcMain.handle('/debug/faceit-result', async (_, payload: { outcome: 'W' | 'D' | 'L'; style?: 'DEFAULT' | 'MVP' | 'BOTTOM' }) => {
    const prisma = await DatabaseClient.connect();
    ...
    const room = reorderRoomForUser(roomBase, profile.playerId);

    const mapPool = await prisma.mapPool.findMany({
      where: {
        gameVersion: { slug: Constants.Game.CSGO },
      },
          ...
          create: [
            {
              ...
              seed: 0,
              score: userScore,
              result: outcome === 'W' ? Constants.MatchResult.WIN : outcome === 'D' ? Constants.MatchResult.DRAW : Constants.MatchResult.LOSS,
            },
            {
              teamId: 2,
              seed: 1,
              score: oppScore,
              result: outcome === 'L' ? Constants.MatchResult.WIN : outcome === 'D' ? Constants.MatchResult.DRAW : Constants.MatchResult.LOSS,
            },
          ],
        },
        games: {
    ...
    await prisma.player.update({
      ...
      data: {
        elo: { increment: delta },
        xp: { increment: outcome === 'W' ? 50 : outcome === 'D' ? 25 : 10 },
      },
    });
